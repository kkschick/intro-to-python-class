...
def getWordScore(word, n):
    ...
    # if the length of the word is zero, return 0
    if len(word) == 0:
        return 0
    
    # if not, iterate through the chars in the word and get the score
    else:
        
        # iterate through chars in the word
        for char in word:
            
            # increse score by the value of the char
            score += SCRABBLE_LETTER_VALUES[char]
        
        # multiply the score by the length of the word
        score *= len(word)
        
        # if the user uses all the letters in the word, add 50 to the score
        if len(word) == n:
            score += 50
...
def displayHand(hand):
    ...
    """
    # iterate through the letters in the hand and display them
...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    # make a copy of the hand and call it newHand
    newHand = hand.copy()
    
    # iterate through the chars in the word
    for char in word:
        
        # remove the chars in word from the hand
